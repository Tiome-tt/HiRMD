...
def run_once(seed):
    ...
    X_train, X_temp, y_train, y_temp, pids_train, pids_temp = train_test_split(
        X_list, y_list, pids, test_size=0.2, random_state=seed, stratify=y_list
    )
    X_val, X_test, y_val, y_test, pids_val, pids_test = train_test_split(
        X_temp, y_temp, pids_temp, test_size=0.5, random_state=seed, stratify=y_temp
    )
    ...
    best_val_acc = 0.0
    epochs_no_improve = 0
    best_model_path = f"best_gru_model_seed_{seed}.pth"
    ...
    for epoch in range(num_epochs):
        ...
        print(f"Val Loss: {val_loss:.4f}, Val Acc: {val_acc:.4f}, "
              f"AUC: {val_auc:.4f}, AUPRC: {val_auprc:.4f}, F1: {val_f1:.4f}")
