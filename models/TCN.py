...
class FusionModelTCN(nn.Module):
    def __init__(self, input_size, num_channels, output_size,
                 ...
                 kernel_size=3, dropout=0.2, embed_dim=32, mlp_hidden_dim=64):
        super(FusionModelTCN, self).__init__()
        self.tcn = TemporalConvNet(input_size, num_channels, kernel_size, dropout)
        self.gpt_linear = nn.Linear(gpt_seq_len, embed_dim)
        self.icu_linear = nn.Linear(icu_feature_dim, embed_dim)
        fusion_input_dim = num_channels[-1] + embed_dim + embed_dim
    ...
    def forward(self, x, gpt_feature, icu_feature):
        # TCN expects input of shape [batch_size, input_channels, seq_length]
        x = x.permute(0, 2, 1)
        tcn_out = self.tcn(x)
        tcn_out = tcn_out[:, :, -1]
        gpt_embed = self.gpt_linear(gpt_feature)
        icu_embed = self.icu_linear(icu_feature)
        fusion = torch.cat([tcn_out, gpt_embed, icu_embed], dim=-1)
        out = self.mlp(fusion)
        return out
...
# 5. Modified version of the training model (based on TCN)
...
model = FusionModelTCN(input_size, num_channels, output_size,
...
epochs_no_improve = 0
patience = 10
best_model_path = 'HiRMD/saved_models/best_tcn_model.pth'
