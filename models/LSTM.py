...
from config.config import datasets, Hidden_Size, Num_Layers, Learning_Rate, Num_Epochs, Batch_Size, Dropout, Input_Features
...
def run_once(seed):
    ...
    model = FusionModel(
        ...
        output_size,
        gpt_seq_len=14,
        icu_feature_dim=icu_feature_dim
    ).to(device)
    ...
    epochs_no_improve = 0
    patience = 10
    best_model_path = f"best_lstm_model_seed_{seed}.pth"
