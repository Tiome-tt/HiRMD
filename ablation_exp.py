# Remove GPT Feature
# mimic-iii:epoch=20 mimic-iv:epoch=20 lr =0.0003 eICU:epoch=15 eICU:lr=0.0001 epoch=35
import gc
import logging
import os
import pandas as pd
...
log_file_path = os.path.join(log_dir, 'ablation_woGPT_training.log')
...
class EHRDataset(Dataset):
    def __init__(self, X, y, pids, icu_dict, icu_features):
        self.X = X
        self.y = y
        self.pids = pids
...
# 3. Define the multi-head attention mechanism and model.

class MultiHeadAttentionModel(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, output_size,
                 icu_feature_dim=33 if datasets=="mimic-iv" else 35 if datasets == "mimic-iii" else 12, dropout=Dropout, embed_dim=32, mlp_hidden_dim=64, num_heads=4):
    ...
    def forward(self, x, icu_feature):
        gru_out, _ = self.gru(x)  # gru_out: (batch_size, seq_len, hidden_size*2)

        icu_embed = self.icu_linear(icu_feature)

        icu_embed_expanded = icu_embed.unsqueeze(1).repeat(1, gru_out.size(1), 1)
        fusion = torch.cat([gru_out, icu_embed_expanded], dim=-1)
        ...
        out = self.mlp(attention_out)
        return out

# 4. Definition of training, validation, and testing processes.
...
def train_model(model, train_loader, criterion, optimizer, device):
    ...
    total_correct = 0
    total_samples = 0
    for X_batch, y_batch, icu_batch in train_loader:
        X_batch, y_batch = X_batch.to(device), y_batch.to(device)
        icu_batch = icu_batch.to(device)

        optimizer.zero_grad()
        outputs = model(X_batch, icu_batch)
...
def evaluate_model(model, loader, criterion, device):
    ...
    all_labels = []
    with torch.no_grad():
        for X_batch, y_batch, icu_batch in loader:
            X_batch, y_batch = X_batch.to(device), y_batch.to(device)
            icu_batch = icu_batch.to(device)
            outputs = model(X_batch, icu_batch)
            loss = criterion(outputs.squeeze(), y_batch)
...
for fold, (train_idx, val_idx) in enumerate(skf.split(X_list, y_list)):
    ...
    try:
        ...
        train_dataset = EHRDataset(X_train_padded, y_train_fold, [pids[i] for i in train_idx],
                                   icu_scores_dict, icu_features)
        val_dataset = EHRDataset(X_val_padded, y_val_fold, [pids[i] for i in val_idx],
                                 icu_scores_dict, icu_features)
...
test_dataset = EHRDataset(X_test_padded, y_test, pids_test, icu_scores_dict, icu_features)
test_loader = DataLoader(test_dataset, batch_size=Batch_Size, shuffle=False)

test_loss, test_acc, test_auc, test_auprc, test_f1 = evaluate_model(model, test_loader, criterion, device)
print(
    f"Test Set Results - Loss: {test_loss:.4f}, Acc: {test_acc:.4f}, AUC: {test_auc:.4f}, AUPRC: {test_auprc:.4f}, F1: {test_f1:.4f}")
...
for fold in range(num_folds):
    plt.plot(epochs, train_losses_all_folds[fold], label=f'Fold {fold + 1} Train Loss')
    plt.plot(epochs, val_losses_all_folds[fold], label=f'Fold {fold + 1} Val Loss')
...
# # Remove ICU Feature mimic-iii and eICU and mimic-iv:epoch=20 lr=0.0003
# import gc
# import logging
# import os
# import json
...
# os.makedirs(log_dir, exist_ok=True)
#
# log_file_path = os.path.join(log_dir, 'ablation_woICU_training.log')
# logging.basicConfig(filename=log_file_path, level=logging.INFO, format='%(asctime)s %(message)s')
# # 1. Data loading and preprocessing
#
# data_file = f"HiRMD/datasets/{datasets}/processed/EHR_{datasets}.csv"
...
#
# class EHRDataset(Dataset):
#     def __init__(self, X, y, pids, gpt_dict):
#         self.X = X
#         self.y = y
#         self.pids = pids
...
#
#
# train_dataset = EHRDataset(X_train_resampled, y_train_resampled, pids_train_resampled, gpt_dict)
# val_dataset = EHRDataset(X_val_padded, torch.tensor(y_val, dtype=torch.float32), pids_val, gpt_dict)
# test_dataset = EHRDataset(X_test_padded, torch.tensor(y_test, dtype=torch.float32), pids_test, gpt_dict)
#
# train_loader = DataLoader(train_dataset, batch_size=Batch_Size, shuffle=True)
# val_loader = DataLoader(val_dataset, batch_size=Batch_Size, shuffle=False)
# test_loader = DataLoader(test_dataset, batch_size=Batch_Size, shuffle=False)
...
# class MultiHeadAttentionModel(nn.Module):
#     def __init__(self, input_size, hidden_size, num_layers, output_size,
#                  gpt_seq_len=14, dropout=Dropout, embed_dim=32, mlp_hidden_dim=64, num_heads=4):
#         super(MultiHeadAttentionModel, self).__init__()
#         self.gru = nn.GRU(input_size, hidden_size, num_layers, batch_first=True, dropout=dropout, bidirectional=True)
#         self.gpt_linear = nn.Linear(gpt_seq_len, embed_dim)
#         self.multihead_attention = nn.MultiheadAttention(embed_dim=hidden_size * 2, num_heads=num_heads, dropout=dropout)
#
#         final_fusion_dim = hidden_size * 2 + embed_dim
#         self.mlp = nn.Sequential(
...
#                 nn.init.constant_(m.bias, 0.0)
#
#     def forward(self, x, gpt_feature):
#         gru_out, _ = self.gru(x)  # gru_out: (batch_size, seq_len, hidden_size*2)
#
#         fusion = gru_out.permute(1, 0, 2)
#         attention_out, _ = self.multihead_attention(fusion, fusion, fusion)
#         attention_out = attention_out.permute(1, 0, 2).mean(dim=1)
#
...
#     total_samples = 0
#
#     for X_batch, y_batch, gpt_batch in train_loader:
#         X_batch, y_batch = X_batch.to(device), y_batch.to(device)
#         gpt_batch = gpt_batch.to(device)
#
#         optimizer.zero_grad()
#
#         outputs = model(X_batch, gpt_batch)
#
#         loss = criterion(outputs.squeeze(), y_batch)
#         loss.backward()
#
...
#
#     with torch.no_grad():
#         for X_batch, y_batch, gpt_batch in loader:
#             X_batch, y_batch = X_batch.to(device), y_batch.to(device)
#             gpt_batch = gpt_batch.to(device)
#
#             outputs = model(X_batch, gpt_batch)
...
#         val_dataset = EHRDataset(X_val_padded, y_val_fold, [pids[i] for i in val_idx], gpt_dict)
#         train_loader = DataLoader(train_dataset, batch_size=Batch_Size, shuffle=True)
#         val_loader = DataLoader(val_dataset, batch_size=Batch_Size, shuffle=False)
#
#         model = MultiHeadAttentionModel(input_size, hidden_size, num_layers, output_size,
...
#         optimizer = optim.Adam(model.parameters(), lr=learning_rate)
#
#         train_losses_current_fold = []
#         val_losses_current_fold = []
#
#         # Save the model with the best performance of the validation set
#         best_val_auc = 0.0
#         best_model_path = os.path.join(model_save_dir, f"{datasets}_best_model_fold_{fold + 1}.pth")
#
#         # Train
...
#                 f"{datasets}, Fold {fold + 1}, Epoch {epoch + 1}/{num_epochs}, Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}, Val AUC: {val_auc:.4f}")
#
#             train_losses_current_fold.append(train_loss)
#             val_losses_current_fold.append(val_loss)
#
#             # If the current model performs better, then save the model.
#             if val_auc > best_val_auc:
#                 best_val_auc = val_auc
#                 torch.save({
#                     'model_state_dict': model.state_dict(),
...
#                 print(f"Fold {fold + 1}: New best model saved with AUC: {val_auc:.4f}")
#
#         train_losses_all_folds.append(train_losses_current_fold)
#         val_losses_all_folds.append(val_losses_current_fold)
#
#         # Load the best model and evaluate it on the validation set.
#         checkpoint = torch.load(best_model_path)
#         model.load_state_dict(checkpoint['model_state_dict'])
...
# for fold, (val_acc, val_auc, val_auprc, val_f1) in enumerate(fold_results):
#     if val_acc is not None:
#         print(f"Fold {fold + 1}: Val Acc: {val_acc:.4f}, AUC: {val_auc:.4f}, AUPRC: {val_auprc:.4f}, F1: {val_f1:.4f}")
#     else:
#         print(f"Fold {fold + 1}: No valid results.")
#
# if len(fold_results) == 0 or all(result[1] is None for result in fold_results):
#     raise ValueError("No valid fold results. Please check the training process.")
#
...
# X_test_padded = pad_sequence(X_test, batch_first=True, padding_value=0.0)
# y_test = torch.tensor(y_test, dtype=torch.float32)
# test_dataset = EHRDataset(X_test_padded, y_test, pids_test, gpt_dict)
# test_loader = DataLoader(test_dataset, batch_size=Batch_Size, shuffle=False)
#
# test_loss, test_acc, test_auc, test_auprc, test_f1 = evaluate_model(model, test_loader, criterion, device)
# print(
...
#
# for fold, train_losses in enumerate(train_losses_all_folds):
#     plt.plot(train_losses, label=f'Fold {fold + 1} Train Loss')
#
# for fold, val_losses in enumerate(val_losses_all_folds):
#     plt.plot(val_losses, label=f'Fold {fold + 1} Val Loss')
#
# plt.xlabel('Epoch')
# plt.ylabel('Loss')
# plt.title(f'Training and Validation Loss per Fold on {datasets}')
