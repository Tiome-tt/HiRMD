...
# 引入配置和你刚才保存的 iTransformer
from config.config import Input_Features, datasets as ds
from itransformer import iTransformer_Fusion
...
def main():
    ...
    for patient_id, group in grouped:
        ...
        max_len_in_data = max(max_len_in_data, len(group))
        demo_list.append(torch.tensor(group[demo_features].iloc[0].values.astype(np.float32), dtype=torch.float32))

    # 取数据集中最大的序列长度，作为 iTransformer 投影维度
    global_max_seq_len = max_len_in_data

    seeds = [42, 1024, 2023, 8888, 9999]
    all_results = []
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    print(f"Data loaded successfully! Global max sequence length is {global_max_seq_len}.")
    print(f"Starting {len(seeds)} iTransformer runs on {device}...")
    ...
    for run_idx, current_seed in enumerate(seeds):
        ...
        # 实例化完全对齐核心的 iTransformer_Fusion
        model = iTransformer_Fusion(
            input_dim=len(seq_features),
            max_seq_len=global_max_seq_len,  # 传入全局最大长度
            d_model=64,
            n_heads=4,
            e_layers=2,
            d_ff=256,
            demo_dim=len(demo_features),
            gpt_dim=14,
        ...
        # 模型保存路径修改为 itransformer
        best_model_path = f"/home/user1/MXY/TimeAttention/baselines/itransformer_best_model_seed_{current_seed}.pth"
